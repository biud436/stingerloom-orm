...
- **`@Entity({ schema })`: pin a table to a PostgreSQL schema, so shared tables stay reachable inside a tenant context.** Under `search_path` and `schema_qualified` every table followed the active tenant — `SET LOCAL search_path TO "acme"` has no public fallback and `schema_qualified` rewrote the name to `"acme"."plan"` — so a table shared by all tenants (plans, countries, the tenants table itself) raised `relation does not exist` from inside `MetadataContext.run(tenant)`, and the only workaround was cloning it into every tenant schema. A pinned entity is now always addressed as `"schema"."table"`, whatever tenant is active and under every strategy: reads, writes, JOINs, the query builders and `em.ref()`. Synchronize creates the table in that schema (creating the schema when missing) through a driver view bound to it, `SchemaDiff` / `migrate:generate` introspect it there, cross-schema foreign keys spell out the referenced schema, and `PostgresTenantMigrationRunner` leaves pinned tables in the source schema instead of cloning them. `@NonTenantEntity()` now means "global table" under the schema strategies too: it pins the entity to the connection's default schema, so one decorator covers `tenant_column`, `search_path` and `schema_qualified` alike (previously it had no effect outside `tenant_column`, and the table was cloned per tenant). Children of an inheritance hierarchy inherit the root's schema unless they set their own. Code-first: `defineEntity(name, fields, { schema })` and `EntitySchema({ schema })`. Ignored on MySQL and SQLite. New exports: `getEntitySchema()`, `EntityManager.resolveEntitySchema()`; `ISqlDriver` gains the optional `withSchema()` and a sixth `addForeignKey()` argument (`foreignTableSchema`), both PostgreSQL-only.
