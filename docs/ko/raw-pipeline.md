...
로컬 네트워크의 실제 PostgreSQL과 MySQL에서 동일한 벤치마크를 실행했습니다. SQLite 인메모리 벤치마크와 결과가 매우 다릅니다. 그 이유를 이해하는 것이 올바른 선택을 하는 데 중요합니다.
...
### 원격 DB에서 pipe()가 em.find()보다 느린 이유

직관에 반합니다. `pipe()`는 Entity 변환을 건너뛰니까 더 빨라야 하는데, 원격 데이터베이스에서는 2-5배 *느립니다*. 이유는 다음과 같습니다.

병목은 변환이 아니라 **쿼리 횟수**입니다:

```
em.find()       →  쿼리 1회   (SELECT * FROM "users")
pipe(bs=1000)   →  쿼리 101회  (SELECT ... LIMIT 1000) × 100 + 빈 결과 체크 1
```

각 쿼리는 완전한 네트워크 왕복입니다: 클라이언트가 TCP로 SQL을 보내고, 서버가 파싱하고 실행하고, 결과를 돌려보냅니다. 지연이 ~1ms인 로컬 네트워크에서 101번의 왕복은 데이터 전송 전에 ~100ms의 네트워크 오버헤드를 발생시킵니다.

SQLite(인프로세스, 네트워크 없음)에서는 왕복 비용이 없으므로 Entity 변환을 건너뛰는 CPU 절감만큼 `pipe()`가 이깁니다. 원격 데이터베이스에서는 100번의 추가 쿼리에 대한 네트워크 비용이 CPU 절감을 훨씬 초과합니다.

이것은 배치 스트리밍에 내재된 근본적인 트레이드오프입니다:

| | em.find() | pipe() |
|---|---|---|
| 쿼리 수 | 1 | N (100K행 / batchSize) |
| 네트워크 왕복 | 1 | N |
| 피크 메모리 | 모든 행 한 번에 | 배치 1개분 |
| Entity 변환 | 있음 (CPU 비용) | 없음 (CPU 절감) |

### 원격 DB에서 pipe()의 진짜 가치: 메모리 제어

위 수치는 pipe() 결과를 전부 하나의 배열에 모은 것(`collect()`)입니다 — 배치 처리의 의미가 사라진 상태입니다. 실제 사용에서는 각 배치를 처리하고 버립니다:

```typescript
// 전체 행 수와 관계없이 ~27 MB 사용
for await (const batch of em.pipe(User, { batchSize: 5000 }).raw()) {
  await sendToExternalService(batch);
  // 이 이터레이션이 끝나면 batch가 GC됨
}

// 100K행에서 ~44 MB, 1M행에서 ~440 MB 사용
const all = await em.find(User);
```

100K행에서 `em.find()`는 44 MB를 한 번에 로드합니다. 1M행이면 ~440 MB. 10M행이면 메모리 부족. `pipe()`는 행 수에 관계없이 ~27 MB를 유지합니다. 이것이 핵심입니다.

::: tip 원격 DB에서 pipe() vs em.find() 선택 가이드
- **50K행 이하, 메모리에 들어감:** `em.find()` 사용. 단일 쿼리라 더 빠름.
- **50K행 초과, 또는 메모리 제한 환경:** `pipe()` 사용. 벽시계 시간은 느리지만 메모리가 일정.
- **외부 시스템으로 전달 (gRPC, CSV 등):** `pipe()` 사용. 어차피 외부 싱크로 스트리밍하므로 배치당 오버헤드가 분산됨.
- **raw Buffer가 필요:** `pipe().binary()` 사용. 드라이버 레벨 바이너리 데이터를 받을 수 있는 유일한 방법.

대규모 데이터셋 스트리밍 시 처리량을 최대화하려면 `batchSize`를 늘려 왕복 횟수를 줄이세요 (예: `batchSize: 10000` 또는 `batchSize: 50000`).
:::
...
성능은 드라이버에 따라 크게 다릅니다. SQLite는 인프로세스(네트워크 없음), PostgreSQL과 MySQL은 원격(네트워크 왕복)입니다. 선택 전에 세 가지 모두 읽어보세요.

### SQLite (인메모리, 네트워크 없음)

전체 결과를 메모리에 수집. 행당 6컬럼, 5회 실행 중앙값.

| 방식 | 100K 시간 | 100K 메모리 | 처리량 |
|------|-----------|-------------|--------|
...
SQLite는 네트워크 오버헤드가 없으므로 `pipe()`가 `em.find()`보다 **4.2배 빠릅니다** — Entity 변환을 건너뛴 것만으로 이 차이가 납니다.

### PostgreSQL (원격 서버, ~1ms 네트워크 지연)
...
### 원격 DB에서 pipe()가 느린 이유

PostgreSQL과 MySQL에서 `pipe()`는 Entity 변환을 건너뛰는데도 `em.find()`보다 2-5배 *느립니다*. 병목은 변환이 아니라 **쿼리 횟수**입니다:

```
em.find()       →  쿼리 1회    (SELECT *)
pipe(bs=1000)   →  쿼리 101회   (SELECT ... LIMIT 1000) × 100 + 빈 결과 체크 1
```

각 쿼리가 완전한 TCP 왕복입니다. 101번 왕복 × ~1ms = 데이터 전송 전에 ~100ms의 네트워크 오버헤드. SQLite(인프로세스)에서는 이 비용이 0입니다.

### Stream-and-Discard: pipe()가 진짜 이기는 곳

위 벤치마크는 pipe() 결과를 전부 배열에 모았습니다 — 배치의 의미가 사라진 상태입니다. 실제 사용에서는 각 배치를 처리하고 버립니다. 핵심 지표는 **MaxBatch**: 한 시점에 보유하는 최대 데이터량입니다.

**PostgreSQL — 100,000행:**

| 방식 | 시간 | MaxBatch (한 시점 보유량) | 전체 행 |
|------|------|-------------------------|---------|
...
100K행에서 `em.find()`는 **17 MB**를 한 번에 보유합니다. `pipe()`는 **0.17 MB** — 100배 적습니다.

1M행이면 `em.find()`는 ~170 MB. 10M행이면 ~1.7 GB. `pipe()`는 행 수에 관계없이 0.17 MB를 유지합니다. 이것이 핵심입니다.

### 수치를 올바르게 읽는 법

::: warning pipe()와 em.find()를 속도만으로 비교하지 마세요
`pipe()`는 `em.find()`의 대체제가 아닙니다. 서로 다른 문제를 해결합니다:

- **`em.find()`** 는 한 번의 빠른 쿼리로 모든 것을 메모리에 올립니다. 결과가 RAM에 들어가고 Entity 인스턴스가 필요할 때 사용하세요.
- **`pipe()`** 는 메모리가 제한된 배치로 데이터를 스트리밍합니다. 결과가 RAM에 안 들어가거나, 외부 싱크(gRPC, CSV 파일, 메시지 큐)로 전달할 때 사용하세요.

원격 DB에서 `pipe()`는 쿼리가 더 많으므로 벽시계 시간은 항상 느립니다. 하지만 순간 메모리 사용량이 100배 적으며, 데이터셋이 가용 RAM을 초과할 때는 이것만이 중요합니다.
:::
