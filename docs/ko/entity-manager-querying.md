...
이 페이지에서는 EntityManager를 통한 **데이터 조회**를 다뤄요: 컬럼 선택, 정렬, 필터링, 페이지네이션 전략, 집계, 쿼리 분석까지 모두 포함돼요.

기본 CRUD는 [CRUD 기초](./entity-manager.md)를, 쓰기와 트랜잭션은 [쓰기 & 트랜잭션](./entity-manager-writes.md)을 참고해 주세요.

---

## Stingerloom에서 쿼리가 동작하는 방식

각 기능을 살펴보기 전에, `em.find()`를 호출하면 내부에서 어떤 일이 일어나는지 이해하면 도움이 돼요.

ORM은 하나의 문제를 풀기 위해 존재해요: **개발자는 객체로 생각하지만, 데이터베이스는 테이블로 생각한다는 것**. `em.find(User, { where: { name: "Alice" } })`를 작성하면, ORM이 이 객체 기반 요청을 SQL로 변환하고, 데이터베이스에 전송한 뒤, 결과 행을 다시 TypeScript 객체로 만들어 줘요.
...
1. **값은 절대 SQL 문자열에 직접 연결되지 않아요.** `"Alice"` 문자열은 파라미터 플레이스홀더가 돼요 (PostgreSQL에서는 `$1`, MySQL에서는 `?`). 이 방식으로 ORM이 SQL injection을 방지해요 -- 데이터베이스 자체가 "코드"와 "데이터"를 분리하거든요.

2. **식별자(테이블/컬럼명)는 다이얼렉트별로 이스케이프돼요.** PostgreSQL은 큰따옴표(`"user"`), MySQL은 백틱(`` `user` ``)을 사용해요. 이건 데이터베이스 드라이버에 따라 ORM이 자동으로 처리하기 때문에 신경 쓸 필요 없어요.

아래에서 설명하는 모든 기능은 결국 어떤 SQL이 생성되는지를 제어하는 방법이에요. 모든 예제에서 정확한 SQL을 함께 보여줄게요.

---

## SELECT 특정 컬럼

기본적으로 `find()`와 `findOne()`은 모든 컬럼을 가져와요 -- `SELECT *`와 같아요. 편리하지만 몇 개의 필드만 필요할 때는 낭비예요. `User` 테이블에 20개 컬럼이 있는데 API에서 `id`와 `name`만 반환한다면, 매 행마다 18개의 불필요한 컬럼이 네트워크를 타게 돼요.
...
`orderBy`가 없으면 데이터베이스는 **순서를 보장하지 않아요**. 대부분의 데이터베이스가 삽입 순서로 반환하는 것처럼 보이기 때문에 안정적이라고 착각하기 쉬운데, 테이블 재구축이나 병렬 쿼리 이후 갑자기 순서가 바뀔 수 있어요. 순서가 중요하면 항상 `orderBy`를 지정해 주세요.
...
개별 행이 아니라 요약이 필요할 때가 있어요: "사용자가 몇 명이에요?", "평균 주문 금액이 얼마예요?", "최고 점수는?" 이런 것들이 **집계 함수**예요. 데이터베이스가 모든 행을 가져와서 JavaScript에서 계산하는 것보다 훨씬 효율적으로 처리해요.
