...
::: tip 코드 우선 스타일이 더 좋다면?
이 페이지는 **데코레이터** API(`@Entity`, `@Column`, …)를 다뤄요. 데코레이터를 건너뛰고 엔티티 타입을 스키마에서 추론하고 싶다면, [**엔티티 정의하기**](./define-entity.md) — `defineEntity` 빌더를 참고하세요. 두 스타일은 동일한 메타데이터를 생성하고 자유롭게 함께 쓸 수 있어요.
:::
...
> **이미 있는 테이블에서** `synchronize`는 새로 추가한 인덱스는 만들지만, 기존 인덱스를 지우거나 다시 정의하지는 않습니다. 엔티티에서 인덱스를 빼거나 컬럼을 바꿨다면 옛 인덱스는 migration으로 지우세요. 남은 유니크 인덱스는 계속 행을 거부합니다. `migrate:generate`는 기존 테이블의 인덱스 DDL을 아예 작성하지 않아요. [Schema Diff가 비교하지 않는 것](./migrations.md#schema-diff가-비교하지-않는-것)을 참고하세요.
...
두 명의 고객 서비스 상담원이 오전 10:00에 같은 주문을 연다고 상상해 보세요. 상담원 A가 10:01에 상태를 "배송됨"으로 변경해요. 상담원 B는 여전히 이전 데이터를 보고 있고, 10:02에 상태를 "환불됨"으로 변경해요. 상담원 B의 업데이트가 상담원 A의 변경을 아무 경고 없이 덮어써요. 이게 **갱신 분실 문제(lost update problem)** 예요.
