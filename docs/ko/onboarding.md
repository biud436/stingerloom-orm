...
유닛 테스트는 72개 이상의 파일에 1400개 이상의 케이스로 구성돼 있어요. 통합 테스트는 `INTEGRATION_TEST=true` 환경변수가 없으면 자동으로 건너뛰어요.
...
| [Manual Testing Guide](./manual-testing-guide.md) | 수동 통합 테스트 실행 | 통합 테스트 환경을 설정할 때 |
| [Pre-Release Checklist](./pre-release-checklist.md) | 릴리스 전 확인 항목 | 버전 릴리스를 준비할 때 |
