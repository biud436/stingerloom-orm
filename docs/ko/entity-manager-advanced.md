...
한 번만 호출한다면 이 비용은 신경 쓰지 않아도 돼요. 그런데 **같은 모양의 쿼리가 수만 번 반복**된다면 얘기가 달라져요. 반복이 많은 작업 -- 백그라운드 워커의 반복문, 대량 배치 작업 안에서 한 행씩 조회하는 코드, 주기적인 지표 수집 루프 -- 에서는 1~4번 과정이 눈에 띄지 않게 CPU 시간을 꽤 잡아먹어요. 이때 느려지는 원인은 **네트워크 왕복이 아니에요**. **Node.js 프로세스 안쪽에서 SQL을 만드는 일**이 병목이 되는 거예요.
