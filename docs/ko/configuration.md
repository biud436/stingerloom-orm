...
- **`min: 5`** -- 아무도 안 써도 항상 5개의 연결을 열어 둬요. 한동안 트래픽이 없다가 몰릴 때 생기는 콜드 스타트 부담을 없애 줘요.
...
- **식별자 길이가 중요해요.** PostgreSQL은 식별자를 63바이트에서 자름. `DefaultNamingStrategy`는 이를 지키기 위한 해시 폴백 경로가 있어요 — 커스텀 전략에서도 이를 복제하지 않으면 마이그레이션이 의도치 않게 제약 이름을 바꿔 버릴 수 있어요.
