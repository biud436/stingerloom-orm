 ...
 * Upsert 통합 테스트
 *
 * 실제 데이터베이스에 연결하여 EntityManager의 upsert() 메서드를 검증합니다.
...
(SKIP ? describe.skip : describe).each(getTestDrivers())(
  ...
  ({ type, options }: TestDriverConfig) => {
    ...
    beforeAll(async () => {
      ...
      await rawQuery(
        createUniqueIndexSql(type, `uq_${testEntity.tableName}_slug`, testEntity.tableName, "slug"),
      );
    }, 30000);
