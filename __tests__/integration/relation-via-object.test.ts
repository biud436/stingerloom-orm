  ...
  () => {
    ...
    describe("Senior — 엣지 케이스·데이터 무결성", () => {
      it("findOne 결과의 eager 로드된 관계를 변경하고 save()하면 FK가 바뀌어야 한다", async () => {
        ...
        await childRepo.save({ title: "Found child", parentFk: parent1.id });

        // findOne으로 조회 (eager → parent 객체 로드됨)
        const allChildren = await childRepo.find({});
        const list = Array.isArray(allChildren) ? allChildren : [allChildren];
        ...
        // DB 직접 검증
        const rows = await rawQuery(
          `SELECT parentFk FROM \`${entities.childTableName}\` WHERE id = ${found.id}`,
        );
        const row = (rows?.results ?? rows)?.[0];
        expect(Number(row?.parentFk)).toBe(parent2.id);
      });
      ...
      it("findOne 결과의 관계를 null로 설정하고 save()하면 FK가 NULL이어야 한다", async () => {
        const parent = await parentRepo.save({ name: "Will be detached" });
        await childRepo.save({ title: "Detachable", parentFk: parent.id });
