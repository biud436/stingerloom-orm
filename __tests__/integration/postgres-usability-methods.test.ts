...
integrationDescribe(
  ...
  () => {
    ...
    beforeAll(async () => {
      conn = await createTestConnection(
        ...
        () => {
          ...
          // Dedicated write target for insertManyAndReturn: SERIAL PK + two
          // distinguishing non-PK columns (label / seq) so input order can be
          // asserted independently of the generated id. Lives in SCHEMA, so the
          // afterAll DROP SCHEMA ... CASCADE removes it (table + sequence).
          @Entity({ name: `um_batch_return_${SFX}` })
    ...
    // PostgreSQL natively supports RETURNING, so the generated SERIAL PKs come
    ...
    describe("EntityManager.insertManyAndReturn", () => {
      it("returns inserted entities with real generated PKs, in input order, persisted", async () => {
        ...
        // 2) every element carries a real, defined, integer PK > 0 (PG SERIAL
        //    int4 comes back from `pg` as a JS number).
        ...
        // 4) input ORDER preserved — element i corresponds to input i, asserted
        //    on the distinguishing non-PK columns (label + seq), not the PK.
        expect(inserted.map((r) => r.label)).toEqual([
          "alpha",
          "beta",
          "gamma",
        ]);
        ...
        // 5) rows are actually persisted — findByPK round-trips each one with
        //    the same generated PK and column values.
        for (const row of inserted) {
